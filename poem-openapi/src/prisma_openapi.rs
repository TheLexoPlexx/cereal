...
async fn get_system(&self, pool: Data<&PgPool>) -> Result<Json<Vec<System>>> {
  let get = sqlx::query_as!(System, "SELECT * FROM \"System\"").fetch_all(pool.0).await.expect("Failed to get System");
  Ok(Json(get))
}
...
async fn post_system(
  ...
  match sqlx::query("INSERT INTO \"System\" (id, label, order_number) values ($1, $2, $3)").bind(&input.id).bind(&input.label).bind(&input.order_number).execute(pool.0).await {
  Ok(x) => {
...
async fn get_serialnumberpart(&self, pool: Data<&PgPool>) -> Result<Json<Vec<Serialnumberpart>>> {
  let get = sqlx::query_as!(Serialnumberpart, "SELECT * FROM \"Serialnumberpart\"").fetch_all(pool.0).await.expect("Failed to get Serialnumberpart");
  Ok(Json(get))
}
...
async fn get_project(&self, pool: Data<&PgPool>) -> Result<Json<Vec<Project>>> {
  let get = sqlx::query_as!(Project, "SELECT * FROM \"Project\"").fetch_all(pool.0).await.expect("Failed to get Project");
  Ok(Json(get))
}
...
async fn post_project(
  ...
  match sqlx::query("INSERT INTO \"Project\" (id, project_number, description) values ($1, $2, $3)").bind(&input.id).bind(&input.project_number).bind(&input.description).execute(pool.0).await {
  Ok(x) => {
...
async fn get_order(&self, pool: Data<&PgPool>) -> Result<Json<Vec<Order>>> {
  let get = sqlx::query_as!(Order, "SELECT * FROM \"Order\"").fetch_all(pool.0).await.expect("Failed to get Order");
  Ok(Json(get))
}
...
async fn post_order(
  ...
  match sqlx::query("INSERT INTO \"Order\" (order_number, project_id) values ($1, $2)").bind(&input.order_number).bind(&input.project_id).execute(pool.0).await {
  Ok(x) => {
...
async fn get_finalcustomer(&self, pool: Data<&PgPool>) -> Result<Json<Vec<FinalCustomer>>> {
  let get = sqlx::query_as!(FinalCustomer, "SELECT * FROM \"FinalCustomer\"").fetch_all(pool.0).await.expect("Failed to get FinalCustomer");
  Ok(Json(get))
}
...
async fn post_finalcustomer(
  ...
  match sqlx::query("INSERT INTO \"FinalCustomer\" (id, sap_id, name, alias) values ($1, $2, $3, $4)").bind(&input.id).bind(&input.sap_id).bind(&input.name).bind(&input.alias).execute(pool.0).await {
  Ok(x) => {
...
async fn get_factory(&self, pool: Data<&PgPool>) -> Result<Json<Vec<Factory>>> {
  let get = sqlx::query_as!(Factory, "SELECT * FROM \"Factory\"").fetch_all(pool.0).await.expect("Failed to get Factory");
  Ok(Json(get))
}
...
async fn post_factory(
  ...
  match sqlx::query("INSERT INTO \"Factory\" (id, customer_id, name) values ($1, $2, $3)").bind(&input.id).bind(&input.customer_id).bind(&input.name).execute(pool.0).await {
  Ok(x) => {
